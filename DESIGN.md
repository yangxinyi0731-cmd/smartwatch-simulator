...
- **记忆点：** 三模型产品以“真实性与来源状态条”串起来源与回放；独立 E0 检测台以居中的功能性手表设备串起案例输入、检测控制和结果，再用“动作示意 + 实际信号/规律时间图 + 四项格式化事实”解释判断依据。105 组案例库与四步判断过程始终可见，未来数秒风险能力明确锁定。
- **克制区域：** 侧栏、案例列表、模型状态和边界说明采用普通软件布局；只让真正承担检测状态的手表设备和判断依据图具有较强视觉识别。动作图必须标为“示意，不是案例影像”，传感器图必须来自本机已登记文件，规律案例必须改用合成事件时间图；不使用装饰性图表、随机波形或持续动画。
...
独立 E0 检测台使用固定深色侧栏、顶部设备状态栏和浅色主画布。首屏核心是三列操作区：左侧案例搜索、类型筛选和分页；中间是模拟手表、当前案例与开始/暂停/重置；右侧是所选案例事实、已保存摘要、四步检测流程和外部通知计数。主操作区之后使用一块完整宽度的“判断依据”区域：左侧是明确标注为非证据的动作类型示意，右侧是实际传感器合量波形或合成规律时间图，下方固定列出来源核对、连续输入、比较方式和当前结论。三类现有模块与锁定的未来预测能力紧随其后；长篇数据边界放入原生 `details` 渐进展开。
...
三模型产品的 `AppShell` 仍是其页面壳和主导航唯一所有者。独立 E0 检测台使用隔离的原生应用壳，导航只链接“实时检测、案例回放、判断过程、模型状态、说明与边界”；每项使用一行主名称和一行简体中文用途说明，并以 `aria-current` 标记唯一当前区域。由于“实时检测”和“案例回放”在桌面布局中同处一排，直接点击的 URL 锚点优先决定当前项，避免两个入口争抢同一滚动位置。案例类型按钮使用 `aria-pressed`；切换类型或有效搜索结果时，手表与判断区同步到当前可见首个案例。模型状态用一条连续模块带呈现，并为三个现有模型分别提供一句中文释义，避免 KPI 墙。

`StatusBadge` 统一正常、信息、关注、危险和中性状态；状态点始终配文字。`EmptyState` 说明缺少什么、为什么缺少，以及何时会有内容，但不展示虚构数字。

### Forms and overlays

三模型产品总览继续使用既有共享组件。独立 E0 检测台只提供无持久化副作用的搜索、筛选、分页和播放控制。开始/暂停/重置只驱动前端阶段状态；WEDA 案例完成后显示仓库已保存摘要，CAPTURE-24 与合成规律仍只显示“案例就绪”，不得把标签或时间图包装成新模型成绩。选中案例时，8010 本机服务可以只读查询现有 SQLite 登记并重新核对传感器文件 SHA-256、数组形状、类型和有限值；它只返回最多 240 个实际抽样位置对应的加速度/角速度合量，不运行三个模型、不写 SQLite。合成规律案例直接读取固定种子事件文件，明确不生成波形。后端继续强制只演示、不报警。两个界面都禁止浏览器原生 `alert`、`confirm` 和 `prompt`；任何会写个人数据、改变通知、授权收件人或产生外部副作用的控件仍不属于当前系统。
...
使用“正在读取”“案例已就绪”“等待检测”“检测完成”“未检测到跌倒动作”等主动、可核验的表述。连接状态来自本机接口，不从静态文案推断。不使用“开启守护”“智慧洞察”“临床级准确率”等宣传词。案例人物统一称“参与者”，不在主要操作界面用年龄称谓暗示能力范围。WEDA 的保存分值在界面中称“跌倒特征匹配度”，必须紧邻解释“表示动作与受控跌倒动作有多相似，不是现实跌倒概率”；受控模拟跌倒、日常活动误判、CAPTURE-24 恢复子集和合成规律必须分别标记。检测完成后单独生成一段结果分析，只能组合案例类型、输入窗口、保存匹配度和保存记录等已有事实；没有逐轴解释时必须直说，禁止编造某一方向或身体部位“哪里不对”。判断依据中的合量峰值只能描述输入变化，不能写成模型的单一原因；日常活动标签区间使用非危险色，跌倒区间才使用红色语义。未来数秒风险预测固定显示“后端尚未接入 / 锁定”；没有实际输入时不画随机波形或虚构风险曲线。
