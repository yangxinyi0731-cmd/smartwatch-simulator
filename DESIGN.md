...
- **界面类型：** 产品工具，不是营销落地页。
- **记忆点：** 三模型产品以“真实性与来源状态条”把案例来源、真实性类别、文件校验和 manifest 与回放放在同一阅读路径中；独立 E0 工作台以“证据链轨道”把 P0–P2 工程通过和 P3–P9 锁定原因放在同一条时间顺序中。两者都表达可验证性，不承担装饰任务。
...
独立 E0 研究工作台沿用相同侧栏、顶部栏、状态条、圆角和页面宽度。其主阅读顺序固定为真实性边界、证据链轨道、确定性 dry-run、夹具指标、合同/资产审计和原件下载。证据链在窄宽度内由自身横向滚动，不让页面整体横向溢出；深色区域只用于人工分数时间轴，周围合同、Gate 和限制仍使用浅色研究工作台语言。
...
三模型产品总览继续使用既有共享组件。独立 E0 工作台只提供不会持久化的工程策略表单：两个原生 range、连续证据和冷却期原生 select；后端强制 dry-run，提交失败保留参数并显示邻近错误。两个界面都禁止浏览器原生 `alert`、`confirm` 和 `prompt`。任何会写个人数据、改变通知、授权收件人或产生外部副作用的控件仍不属于当前系统。
...
使用“正在检查”“本地服务已连接”“后端未连接”“尚未导入”“暂无数据”“待训练”等主动、可核验的表述。连接状态来自 HTTP 健康检查和 WebSocket，不从静态文案推断。不使用“开启守护”“智慧洞察”“临床级准确率”等宣传词。独立工作台的时间轴只使用版本化人工确定性夹具，并提供可见图例、文字状态、决策表和原始 JSON 下载；所有指标紧邻“工程夹具·不可外推”。未来真实图表仍必须有文字摘要、单位、来源和原始值访问路径；没有真实输入时显示空状态，不画随机波形。
...
- **Don't：** 把 E0 工作台当成 P9 老人风险界面，或用阈值滑块制造“预测已经有效”的视觉暗示。
