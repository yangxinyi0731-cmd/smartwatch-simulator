...
- Primary jobs：先从样本总览选择 105 组公开案例或 30 组自主采集案例中的一组，核对样本详情、运行模拟手表并查看结果；也可上传新的六轴记录。两条路径以同一套六步判断框架完成输入检查、标准化、适用模型实算和结果生成；按“结论—依据—模型推导—边界”核对实际波形与逐秒 1/2/3 秒研究曲线。
...
- Runtime design-system/token source：`frontend/src/tokens.css` 是两个本机界面的共享令牌源；Tabler Core 与 `frontend/src/style.css` 实现三模型产品，`research/early_risk/workbench/app.css` 持有隔离 E0 工作台的局部布局/颜色角色。不得为仅工作台换肤改动产品全局令牌。
...
- Typography：标题与正文统一使用简体中文无衬线系统字体；不再使用宋体展示标题。独立工作台局部先用本机 `Segoe UI Variable`，缺失时回退到共享系统字体栈，不加载远程字体。
- Motion：不使用持续环境动画；只有表达交互状态的短过渡，并支持 `prefers-reduced-motion`。
- E0 空间结构：参考 Nobi 电脑端工作台的深色通栏顶栏、浅色分组侧栏、浅灰主画布和记录网格，但不复制其品牌、个人资料或实时报警语义；首屏是“样本总览”，不是上传表单或产品介绍。
- E0 结果层级：案例判断区与新文件结果区均先显示本次结论卡，再显示案例资料或四项事实、图表；结论卡靠字号、留白和单条状态色顶边突出，次要内容使用较轻的局部边框与行分隔。案例桌面双列只并排手表与简明结论，完整推导、案例资料、匹配度及共用六步流程接在双列后全宽显示，避免一列内容过长产生大片空白；窄屏保持同样的 DOM 阅读顺序。等待/运行中为中性或信息色，需要复核为琥珀色，发现跌倒候选为红色；三种颜色均须有文字，不表示现实报警。
...
| 校赛结果分析 | `analysis.interpretation` + `renderReasonedAnalysis` | 上传与案例的结论卡均先于次要事实和图表，内部共用“一句实际结论 → 三条实际依据 → 可见适用边界 → 原生 details 中的全部依据、模型推导、完整结论和规则”。摘要只选取已有解释字段，不在前端补算结论；未运行模型及原因必须明示，未运行不等于零风险 |
| 校赛投屏阅读密度 | `#presentation-toggle` + `body[data-view]` | 原生按钮保留 44px 触控目标与 `aria-pressed`；只在当前标签页记住开/关。投屏只放大结论、依据和事实，并把新数据结果排成文字优先的单列；不改模型、图表数据或风险语义 |
| 校赛样本详情 | `#case-list` + 原生 `#case-detail` dialog | 一张记录卡一个按钮；选择即同步当前记录并打开详情。来源、采样、传感器、时长和本次筛查分开呈现，登记动作标签不能充当模型输出；关闭后回原按钮，进入检测后聚焦真实操作 |
...
| E0 Case grid | 类型总数 + 3×2 桌面记录网格、每页 6 条 | 卡片背景与边框强调 | 整张按钮可见轮廓 | `aria-pressed` 当前选择；点击打开详情 | 翻页边界禁用 | 选择时尺寸稳定 | 保留已选案例并允许重新读取 |
| E0 Case dialog | 来源、采样、传感器、时长和“尚未运行” | 关闭/进入按钮色阶变化 | 原生 modal 焦点约束与可见轮廓 | 只有实际完成的本次模型结论可替换未运行状态 | 不适用 | 检测中明确提示未出最终结论 | 关闭可回原卡片，进入检测聚焦操作按钮 |
| E0 Watch controls | 开始、暂停、重置 | 色阶变化 | 3px 可见轮廓 | 当前阶段与共用六步流程同步 | 实际案例依据未读取时不启动 | 已知六步确定进度 | 页面级连接错误提供重新连接 |
| E0 结果卡 | 等待时中性文字与边框 | 无交互，不制造悬停暗示 | 检测完成后聚焦结果标题 | 本次计算后按结论分别显示候选红、复核琥珀或非候选状态 | 无输入时不显示危险色 | 运行中只说明正在计算 | 未运行模型和适用边界保持可见 |
| New-data upload | 可见文件标签、拖放区、两个单位选择器 | 边框强调 | 3px 可见轮廓 | 显示文件名和大小 | 忙碌时禁用更换与重复提交 | 稳定按钮文字“正在运行全部模型” | 相邻持久文字错误，保留文件并可重试 |
| E0 投屏模式 | 默认关闭，标签页内刷新保持选择 | 边框及文字变化 | 3px 可见轮廓 | `aria-pressed` 与可见开/关同步 | 不适用 | 不适用 | 存储不可用时仍能切换本页 |
...
- E0 case page size：前端固定 6 条/页；桌面三列记录网格，窄屏降列。案例库显示 105 组公开案例和 30 组自主采集案例，支持单独筛选。切换筛选、翻页或有效搜索结果后，如果原选中项不可见，手表和判断区同步到首个可见案例。数字是登记数量，不是报警计数。
...
| 分析新数据 | “开始分析新数据” | 六步阶段名，不显示假百分比；按钮尺寸稳定 | 当前上传结果区 | 先显示生成结论，再显示四项事实与双证据图 | 保留文件和单位；相邻错误说明后重试 | 成功聚焦结果标题并滚动到结论区，尊重减少动态效果设置；失败聚焦错误区 | `docs/early_risk/WORKBENCH.md` |
| 阅读完整判断 | 两处结果卡片中的“展开完整判断依据与模型推导” | 无 | 当前结果卡片 | 展示原始完整结论、全部证据、每个模型状态及规则，不覆盖三条摘要或边界 | 原生 details 可随时收起；切换案例或重新运行时重置展开状态 | 原生 summary | `analysis.interpretation` |
| 切换投屏模式 | 顶部“投屏模式”按钮 | 无 | 当前页面 | 仅变更阅读密度、保持当前案例和结果；开/关文字与 `aria-pressed` 同步 | 标签页存储不可用时仍保持本页切换 | 原按钮 | `DESIGN.md` |
...
| 筛选 E0 案例 | 六个数据类型按钮：全部/受控跌倒/日常活动/活动识别/生活规律/自主采集 | 无网络等待 | 样本总览 | 网格区间与总数、分页、手表和判断区同步；数字仅为登记案例数 | 无匹配时显示空结果；不伪造案例或报警 | 原筛选按钮 | `docs/early_risk/WORKBENCH.md` |
| 选择 E0 案例 | 样本网格的整卡按钮 | 切换到不同记录时重置上次前端阶段并核对文件；再次点击当前卡片不重置已算结果 | 原生样本详情 dialog | 手表编号、输入规格、事实标签、判断过程和依据图同步；详情显示未运行或本次实际状态 | 读取失败时明确显示“依据不可用”，不生成替代曲线；仍可选择其他案例或重新读取 | 关闭返回原卡片按钮 | `docs/early_risk/WORKBENCH.md` |
| 进入 E0 检测 | 样本详情中的“查看并运行检测” | 无虚构进度 | `#monitor` | 关闭详情、更新位置并保持所选样本 | 若无法打开原生 dialog，则可直接跳到检测区 | 聚焦“开始检测”操作 | 本合同 |
| 开始模拟检测 | “开始检测” | 共用六步依次显示，按钮尺寸稳定；实际依据未就绪时不提前运行 | 当前检测台的结论卡 | WEDA 显示三个传感器模型实算；CAPTURE-24 显示活动模型实算及两项不适用原因；规律案例显示规则实算；结论位于案例资料和匹配度之前 | 可暂停、继续或重置；依据缺失时停止并显示原因；不创建后端写入 | 当地址位于 `#monitor` 或 `#analysis` 时，完成后聚焦结论标题并滚动到判断区；尊重减少动态效果设置 | `docs/early_risk/WORKBENCH.md` |
...
| 读取 E0 判断依据 | 选择案例自动触发 `GET /api/case-evidence/{case_id}` | 保留区域尺寸并显示“正在核对” | 当前检测台 | 返回共用六步状态和结构化解释；WEDA/CAPTURE-24 显示经哈希与形状核对的真实合量波形，规律案例显示固定种子事件时间图 | 404/503 时显示文字错误态；禁止示意曲线、静态结论或旧案例响应覆盖新选择 | 原样本卡片 | `docs/early_risk/WORKBENCH.md` |
...
- 校赛页面导航：深色顶栏承载品牌、当前栏目、本机状态和可操作工具；浅色侧栏按“总览”和“数据与设置”分组，只使用“样本总览、模拟手表、判断过程、信号证据、新数据检测、模型状态、说明与边界”七个真实锚点。唯一 `aria-current` 与顶栏当前位置同步；小于 820px 转成可横向滚动的顶部导航。没有登录/个人账户功能，因此不伪造账户头像或个人资料。公开代理模型显示“已接入”，现实预测证据与 P3–P9 仍只在边界区说明，不伪装成已完成产品能力。
...
- Dialog primitive：现有三模型产品未来第 12 步使用验证过的 Vue 可访问对话框共享组件；独立校赛工作台当前使用原生 `dialog.showModal()` 呈现样本详情，Escape、关闭按钮和点击遮罩可关闭，关闭时焦点返回原卡片；跳转检测时聚焦检测按钮。
- Destructive confirmation：首版无删除；未来可恢复为 warning，不可恢复为 danger。
- Toast：右上稳定位置、去重；成功约 4 秒，错误不只放在 toast。
- Alert/banner：连接中断使用持续页级 banner；校赛页仅显示模型研究筛查状态，不创建现实紧急跌倒报警或外部通知。
...
- 校赛 page states：验证本机内容读取中、读取失败/重试；样本总览六类筛选、三列网格、搜索、分页、样本详情打开/Escape/关闭/焦点返回/进入检测；登记标签为中性色，只有完成六步模型计算才显示本次筛查状态。继续验证上传空状态、合法 CSV、合法 JSON、缺少陀螺仪、错误编码、超大文件、短于 1 秒拒绝、1–4 秒部分模型降级、4–20 秒活动模型降级、20 秒以上全部模型完成、移除和重试；自主采集显示 30/约30组并可逐组回放。成功结果必须显示数据质量、当前动作候选、跌倒筛查、未持久化、加速度/角速度实际波形、1/2/3 秒逐秒曲线、各自阈值、代理锚点说明、判断依据和最终结论。继续验证 105 组公开案例与 30 组自主采集案例的分类、手表控制、模型响应、依据失败、七项导航唯一 `aria-current`、四个模型中文释义和外部通知 0 次。人物统一称“参与者”，不得显示“综合风险分”。1440×900、1024×768 与 390×844 均不得页面级横向溢出，所有可见操作目标至少 44×44px。
