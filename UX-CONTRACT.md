...
- Primary jobs：上传新的六轴记录，或从 105 组公开案例和 30 组自主采集案例中选择一组，以同一套六步判断框架完成输入检查、标准化、适用模型实算和结果生成；按“结论—依据—模型推导—边界”核对实际波形与逐秒 1/2/3 秒研究曲线。
...
| 校赛结果分析 | `analysis.interpretation` + `renderReasonedAnalysis` | 上传与案例共用“一句实际结论 → 三条实际依据 → 可见适用边界 → 原生 details 中的全部依据、模型推导、完整结论和规则”。摘要只选取已有解释字段，不在前端补算结论；未运行模型及原因必须明示，未运行不等于零风险 |
...
- E0 case page size：前端固定 6 条/页；案例库显示 105 组公开案例和 30 组自主采集案例，支持单独筛选。切换筛选、翻页或有效搜索结果后，如果原选中项不可见，手表和判断区同步到首个可见案例。
...
| 筛选 E0 案例 | 六张数据类型概览卡：全部/受控跌倒/日常活动/活动识别/生活规律/自主采集 | 无网络等待 | 当前检测台 | 列表区间与总数、分页、手表和判断区同步；数字仅为登记案例数 | 无匹配时显示空结果；不伪造案例或报警 | 原筛选按钮 | `docs/early_risk/WORKBENCH.md` |
...
- 校赛 page states：验证本机内容读取中、读取失败/重试；上传空状态、合法 CSV、合法 JSON、缺少陀螺仪、错误编码、超大文件、短于 1 秒拒绝、1–4 秒部分模型降级、4–20 秒活动模型降级、20 秒以上全部模型完成、移除和重试；自主采集显示 30/约30组并可逐组回放。成功结果必须显示数据质量、当前动作候选、跌倒筛查、未持久化、加速度/角速度实际波形、1/2/3 秒逐秒曲线、各自阈值、代理锚点说明、判断依据和最终结论。继续验证 105 组公开案例与 30 组自主采集案例的分类、搜索、分页、手表控制、模型响应、依据失败、六项导航唯一 `aria-current`、四个模型中文释义和外部通知 0 次。人物统一称“参与者”，不得显示“综合风险分”。1440×900 与 390×844 均不得页面级横向溢出，所有可见操作目标至少 44×44px。
