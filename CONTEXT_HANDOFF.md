...
## 2026-09-06 最新补充：P01 自主采集数据

- 本轮交付分支为 `data/self-collected-p01`，由远程当前基线建立；没有改写历史，也没有覆盖原工作目录中尚未提交的用户修改。
- P01 自主采集包包含 1 名匿名参与者、6 类受控非跌倒动作、30 组记录。全部完成加速度与角速度对齐、去标识化、50 Hz 标准化和冻结模型工程验证。
- 仓库保存 30 组仅含相对时间与六轴数值的标准文件、逐文件哈希和工程验证报告；原始压缩包、设备信息与绝对采集时间不上传。
- 30 组全部通过质量检查；现有跌倒筛查模型把其中 7 组列为候选，包括 5 组大幅度摆臂和 2 组走路，说明跨设备、跨动作习惯迁移和误报控制仍是下一阶段重点。
- 项目计划书摘要位于 `docs/PROJECT_PLAN_ABSTRACT.md`，已突出团队完成的多来源数据整理、四类研究模块、完整演示链路、超过 100 项检查和真实困难样本复核。
- 只有一名参与者，动作标签来自文件名，压缩包未附视频、第二标注者复核或知情同意文件；不得据此声称现实跌倒预测、跨人群有效或医疗部署完成。
