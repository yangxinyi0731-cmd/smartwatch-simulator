...
- 独立 E0 模拟手表检测台：在 105 组案例上演示选择、检测与结果复核，并用动作类型示意、经本机文件核验的真实合量波形或合成规律时间图展示格式化判断依据；P0–P2 Gate 和 dry-run 安全边界仍保持隔离。
...
这些能力只属于 E0 工程研究骨架，没有接真实个人数据，没有训练提前预测模型，也没有面向老人或家属的风险界面与通知。独立工作台可以只读核对已登记的现有案例输入，但不运行三个模型，也不能把保存摘要、输入波形、合成规律或人工夹具写成现实环境中的提前预测成绩。

完成一次“首次安装”后，可以双击 `scripts\windows\start-early-risk-workbench.cmd` 打开独立研究工作台，地址固定为 `http://127.0.0.1:8010/`。对应的停止和诊断入口为 `stop-early-risk-workbench.cmd` 与 `diagnose-early-risk-workbench.cmd`。工作台不启动现有产品服务；为了显示判断依据，它会只读查询 SQLite 案例登记并重新核对本机传感器文件，但不运行模型、不写 SQLite、不写研究报告，也不产生外部通知。
