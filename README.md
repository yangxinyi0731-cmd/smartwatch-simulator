...
这是一个只在 Windows 电脑上运行的三模型模拟智能手表研究平台。当前已经具备 Tabler 风格 Vue 前端、FastAPI 本地后端、100 个可追溯 WEDA 案例、1 个合成规律案例、WebSocket 实时状态通道、真实回放界面，以及统一案例/来源/回放/三模型合同。跌倒模型和个人规律模型已接入研究版推理；活动识别模型正在取得固定数据并训练。
...
当前公开来源案例实数为 100：40 条年轻参与者受控床垫模拟跌倒、30 条老年参与者受控日常活动、30 条年轻参与者受控日常活动；另有 1 个明确标记为合成数据的 100 天生活规律案例，因此 SQLite 案例总数为 101。它们不是“100 个真实老人跌倒”。活动识别仍在训练，界面中的活动模型“待训练”仍是准确状态。
