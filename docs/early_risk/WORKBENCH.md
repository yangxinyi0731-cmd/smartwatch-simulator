...
`http://127.0.0.1:8010/` 现在是一个可真实运行模型的本机风险评估工作台，而不是产品介绍页。评委既可以选择 105 组已登记公开案例，也可以上传一段新的六轴记录。两种来源共用同一套六步判断框架和同一套结果分析格式：检查输入、统一标准、识别动作或规律、筛查跌倒动作、分析 1/2/3 秒提前风险、生成有依据的结论。
...
自主采集登记现为 `30/约30组`。这 30 组均来自 P01 原始采集包，已完成完整性检查、双传感器对齐、去标识化和冻结模型工程验证；没有用随机数据、复制的公开案例或静态前端结论填充这一栏。
...
| GET | `/api/workbench` | 返回公开模型报告、P01 自主采集30组登记状态、E0 合同和案例摘要 | 否 |
...
登记位于 `data/catalog/self_collected_pending_v1.json`，完整报告位于 `reports/early_risk/self_collected_p01_engineering_validation_v1.json`：

- 目标约 30 组；
- 当前收到并接受 30 组；
- 1 名匿名参与者，6 类受控非跌倒动作，每类 5 组；
- 两路约 100.4 Hz 的加速度与角速度已完成共同时间段对齐，并统一为 50 Hz 六轴数据；
- 30 组均运行完整传感器分析链路；
- 7 组达到跌倒候选筛查条件，其中大幅度摆臂 5 组、走路 2 组；
- 允许表述“完成自主采集数据接入与冻结模型工程验证”，不允许表述为现实预测或跨人群有效性。

原始压缩包不进入 GitHub。仓库只保存去除设备信息和绝对时间后的标准六轴文件、逐文件哈希和工程验证报告。动作标签来自压缩包文件名，未提供视频、第二标注者复核或知情同意文件；后续仍需增加参与者和长期自然活动，复核大幅度摆臂与走路引起的误判。
...
- `self_collected_validation_complete=true` 只表示 P01 数据接入与冻结模型工程验证完成；`real_world_prediction_evidence=false`、`deployment_approved=false`、外部通知 0 次继续保持。
...
浏览器验收必须覆盖：合法上传、缺列失败、短记录降级、文件移除和重试；桌面与 `390×844`；键盘文件选择和焦点回到结果标题；逐秒图、阈值、代理锚点、波形文字摘要；105 组公开案例选择与手表同步；自主采集显示 30/约30组且工程复核完成；控制台无错误且页面无横向溢出。
