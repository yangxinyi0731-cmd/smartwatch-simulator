...
活动数据下载期间，案例目录和真实回放界面已提前完成首个可用闭环：用户可筛选 101 个案例、读取两个已登记研究模型、回放真实六轴文件或 100 天合成事件，并查看波形、标签、候选告警、质量标记和模型证据。当前回放为只读预览，WebSocket 仍只承担系统连接状态。
...
- `.\.venv\Scripts\python.exe -m pytest .\backend\tests -q`：44 个测试通过；
...
- 当前构建产物为 CSS 554.30 kB（gzip 71.83 kB）、JavaScript 101.84 kB（gzip 38.25 kB）；
...
- 尚未完成独立案例库页面、批量测试、报告导出和 Windows 一键交付包；
- 没有创建或推送 GitHub 远程仓库。
...
前端默认地址：`http://127.0.0.1:5173/`；健康检查：`http://127.0.0.1:8000/api/health`；合同：`http://127.0.0.1:8000/api/contracts`；案例：`http://127.0.0.1:8000/api/cases`；模型：`http://127.0.0.1:8000/api/models`；单案例回放：`/api/cases/{case_id}/replay-preview`。
