...
“三个独立模型接入”大关已经完成：跌倒 ONNX 与 100 案例同源重放、100 天合成规律与统计规则、CAPTURE-24 腕部活动 ONNX 与参与者隔离留出评估均已接入。SQLite 结构版本 4 的默认库现有 105 个案例、3 个研究模型 manifest 和 3 份规范化报告。用户已明确授权后续按完整大关连续推进，不需要每个大关再等待确认；只有必须由用户本人处理的登录、许可申请、隐私/法律选择或外部账号操作才暂停。

案例目录、真实回放界面和统一测试报告已经形成三模型可用闭环：用户可筛选 105 个案例、读取三个已登记研究模型，回放真实六轴 WEDA、真实三轴 CAPTURE-24 或 100 天合成事件，并查看波形、标签、候选告警/活动概率、质量标记和模型证据。测试报告从已保存 JSON 读取真实数值，显示适用范围、限制、文件路径和 SHA-256，并可下载统一快照。当前回放仍为只读预览，WebSocket 仍只承担系统连接状态。
...
- 服务版本提升到 `0.6.0`，重启后 `GET /api/health` 应返回结构版本 4 和案例实数 105；
- 以下上一大关成果继续保留：
...
- `.\.venv\Scripts\python.exe -m pytest .\backend\tests -q`：53 个测试通过；
...
- 当前构建产物为 CSS 557.32 kB（gzip 72.30 kB）、JavaScript 107.53 kB（gzip 39.55 kB）；
...
- 运行态 HTTP 实测：服务版本 0.6.0、SQLite 结构版本 4、案例总数 105、模型清单与报告清单各 3 个，均未获部署批准；
...
- 右侧本地前端持续运行并显示 SQLite 结构版本 4、105 个案例和三个模型；
...
- 尚未完成独立案例库页面、可持久恢复的批量重放任务和 Windows 一键交付包；
- 没有创建或推送 GitHub 远程仓库。
...
前端默认地址：`http://127.0.0.1:5173/`；健康检查：`http://127.0.0.1:8000/api/health`；合同：`http://127.0.0.1:8000/api/contracts`；案例：`http://127.0.0.1:8000/api/cases`；模型：`http://127.0.0.1:8000/api/models`；报告：`http://127.0.0.1:8000/api/reports`；报告下载：`http://127.0.0.1:8000/api/reports/export.json`；单案例回放：`/api/cases/{case_id}/replay-preview`。

## 下一大关（已获连续推进授权）

完成可持久恢复的批量重放任务与 Windows 一键本地交付：批量任务必须保存状态、进度、错误和幂等请求标识，重启后能够恢复；本地交付必须只监听 `127.0.0.1`、复用现有数据库和三模型资产，并提供清晰启动/停止/诊断脚本。未完成外部验证的模型全部保持 `deployment_approved=false`。继续保持：
